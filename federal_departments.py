...
truncate_table = "truncate table mrr_federal_departmens;"
sql_query(truncate_table)
print('table mrr_federal_departmens is truncated')

url =  'http://api.duma.gov.ru/api/'+api_token+'/federal-organs.json?app_token='+app_token
print(url)
response = requests.request('GET',url)
result = json.loads(response.text)
for f in range(0, len(result)):
        federal_department_id = result[f]['id']
        department_name = result[f]['name']
        is_current = result[f]['isCurrent']
        start_date = result[f]['startDate']
        end_date = result[f]['stopDate']
        insert_data = "INSERT INTO mrr_federal_departmens\
        (id, department_name, is_current, start_date, end_date)\
       VALUES(" + "'" + str(federal_department_id) + "','" + str(department_name) + "','" + \
                        str(is_current) + "','" + str(start_date)+ "','" + str(end_date) + "');"
        sql_query(insert_data)
        # print(insert_data)
print('table mrr_federal_departmens is updated')